        ...
        <div class="flex items-center justify-center text-blue">
          <div class="relative drop-shadow-lg w-full md:p-6 p-3 bg-light rounded-lg shadow-xl ">
            ...
            <a target="_blank" href="{{route('generate-pdf')}}">
              <button type="button" class="float-left text-white bg-redpink hover:bg-blue focus:ring-4 focus:outline-none font-medium rounded-lg text-base px-6 py-2.5 text-center  mt-5">
                Download PDF
              </button>
            </a>
                    ...
                    @foreach($subjects as $subject)
                      <tr class="bg-slate-50 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-blue/50 hover:text-white dark:hover:bg-blue/50">
                          ...
                          <td class="flex px-6 py-4 gap-4">
                            ...
                            {{-- Modal --}}
                            <dialog id="myModal{{$subject->id}}" value="{{$subject->id}}" class=" w-11/12 md:w-4/12 p-8  bg-white rounded-md ">
                              <div class="flex flex-col w-full">
                                ...
                                <div class="p-4 text-center">
                                  <form method="POST" action="{{ route('admin.delete_subject', $subject->id) }}" class="inline">
                                    @csrf
                                    <button onclick="document.getElementById('myModal{{$subject->id}}').close();" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                        Delete
                                    </button>
                                  </form>

                                  <button onclick="document.getElementById('myModal{{$subject->id}}').close();" class="text-blue bg-light hover:bg-blue rounded-lg border text-sm font-medium px-5 py-2.5 hover:text-light focus:z-10">
                                    Cancel
                                  </button>
                                </div>
