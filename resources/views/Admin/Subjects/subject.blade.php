                    ...
                    @foreach($subjects as $subject)
                      <tr class="bg-slate-50 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-blue/50 hover:text-white dark:hover:bg-blue/50">
                          ...
                          <td class="flex px-6 py-4 gap-4">
                            ...
                            {{-- Modal --}}
                            <dialog id="myModal{{$subject->id}}" value="{{$subject->id}}" class="w-full lg:max-w-[500px] max-w-modal md:max-w-[360px] md:p-8 p-4  bg-white rounded-md">
                              <div class="flex flex-col w-full">
                                ...
                                <div class="md:p-4 px-2 text-center flex justify-between items-center flex-row lg:w-7/12 md:w-10/12 w-[90%] mx-auto">
                                  <form method="POST" action="{{ route('admin.delete_subject', $subject->id) }}" class="inline">
                                    @csrf
                                    <button onclick="document.getElementById('myModal{{$subject->id}}').close();" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                        Delete
                                    </button>
                                  </form>
