        ...
        <div class=" mb-4">
            <div class="flex items-center justify-center text-blue">
                <div class="relative drop-shadow-lg w-full md:px-6 px-3 py-10 bg-light rounded-lg shadow-xl ">
                    <p class="font-semibold text-xl text-center mb-10">Edit Notice</p>

                    <form method="POST" action="{{ route('admin.update_notice', $notice->id) }}" class="md:w-4/5 md:mx-auto">
                        @csrf
                        <div class="mb-6">
                            <label for="title" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">
                                Notice Title
                            </label>
                            <input type="text" id="title" name="title" value="{{ $notice->title }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                @error('title')
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="description" name="description" rows="8" class="block p-2.5 w-full text-sm text-blue bg-gray-50 rounded-lg border">{{ $notice->description }}</textarea>
                            @error('description')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">
                                <span class="font-medium">{{ $message }}</span>
                            </p>
